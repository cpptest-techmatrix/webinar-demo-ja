pipeline {
    ...
    stages {
        ...
        stage('Build Project') {
            steps {
                sh 'docker exec -u 1000 -w ${CONTAINER_HOME_DIR}/${CPPTEST_PROJECT_NAME} -i ${DOCKER_CONTAINER_NAME} /bin/bash -c "cmake . && cpptesttrace --cpptesttraceProjectName=${CPPTEST_PROJECT_NAME} make"'
            }
        }
        stage('Create C++test Project') {
            steps {
                sh 'docker exec -u 1000 -i ${DOCKER_CONTAINER_NAME} cpptestcli -data . -bdf ${CPPTEST_PROJECT_NAME}/cpptestscan.bdf -localsettings cpptest.ls.properties -showdetails'
            }
        }
        stage('Run Tests') {
            parallel {
